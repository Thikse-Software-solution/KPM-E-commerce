...
export class CartService {
  ...
  getCartItems(userId: number): Observable<CartItem[]> {
    return this.http.get<CartItem[]>(`${this.apiUrl}/items/${userId}`).pipe(
  ...
  addOrUpdateCartItem(userId: number, productId: number, quantity: number): Observable<any> {

     // Ensure that the productId, userId, and quantity are defined
  if (productId == null || userId == null || quantity == null) {
    console.error('Invalid parameters for addOrUpdateCartItem:', { userId, productId, quantity });
